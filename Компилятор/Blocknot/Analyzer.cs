...
namespace Blocknot
{
    public class Analyzer
    {
        ...
        private static bool ExpectKeyword(ref int pos, string keyword, ref StringBuilder output, string input, Func<string, bool> AddError)
        {
            ...
            // Проверяем на наличие недопустимых символов — символов, не являющихся английскими буквами
            ...
            for (int i = 0; i < word.Length; i++)
            {
                ...
                {
                    invalidChars.Add(c);
                    // Выведем ошибку для каждого недопустимого символа
                    int errPos = start + i;
                    if (!AddError($"8\tERROR\t\"{c}\"\t({errPos} - {errPos}) -> Недопустимый символ в ключевом слове '{keyword}'."))
                        return false;
                }
            }

            // Если были недопустимые символы — считаем слово недопустимым и возвращаем false
