...
namespace Blocknot
{
    public partial class Form1 : Form
    {
        ...
        private void AnalyzeButtonClick(object sender, EventArgs e)
        {
            ...
            string analysis = Analyzer.Analyze(inputText);

            // 2. Парсим результат анализа
            string parsed = Parser.Parse(analysis);

            // 3. Показываем результат в resultTextBox
            resultTextBox.Text = parsed;

            // 4. Предупреждение, если найдены ошибки
            if (parsed.Contains("Ошибка"))
            {
                MessageBox.Show("Обнаружены ошибки при анализе!", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
